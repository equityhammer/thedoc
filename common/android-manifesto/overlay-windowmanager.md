...
**Key contract points:**
- `announceForAccessibility` is a no-op when no accessibility service is active, so it is safe to call unconditionally when the formatter returns non-null.
...
- The announcement is the user-facing "what just happened?" line. Keep it one sentence pair, no preamble. Material's `Snackbar` equivalent for non-overlay screens would be a regular `Snackbar` with the same text.
...
- Empty-text banners produce empty contentDescription; there is no point announcing a bare "Critical: ".
...
2. `android:singleLine="true"` (the older API; `maxLines="1"` alone does NOT trigger marquee, a known Android quirk)
...
events from a large foldable test device (Android 16 / SDK 36) where the v1.0.100 (UX-50) ripple
